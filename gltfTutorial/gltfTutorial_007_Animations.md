...
In the example above, there are two channels for the animation. Both refer to the same node. The path of the first channel refers to the `rotation` of the node, and the path of the second channel refers to the `translation` of the node. So all objects (meshes) that are attached to the node will be rotated and translated by the animation, as shown in Image 7b.
